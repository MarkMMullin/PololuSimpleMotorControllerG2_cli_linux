...
using SmcG2_linux;
using Usbwrapper_linux;

namespace Pololu.SimpleMotorControllerG2.SmcG2Cmd_linux
{
    class Program
    {
        /// <summary>
        ...
        {
            return "smcg2cmd: Pololu Simple Motor Controller G2 Command-line Utility\n" +
                "Version: " + Assembly.GetExecutingAssembly().GetName().Version.ToString(3) + "\n" +
                "Options:\n" +
                // All lines here should be at most 80 characters.  Character 80 is here........!
                " -l, --list                   List devices conneccted to computer.\n" +
                " -d, --device SERIALNUM       Specifies the serial number of the device.\n" +
                " -s, --status                 Show device status and info.\n" +
                "     --stop                   Stop the motor.\n" +
                "     --resume                 Allow motor to start.\n" +
                "     --speed NUM              Set motor speed (-3200 to 3200).\n" +
                "     --brake                  Immediately start braking.\n" +
                "     --coast                  Immediately start coasting.\n" +
                "     --current-limit          Set current limit in milliamps.\n" +
            ...
            while(argEnumerator.MoveNext())
            {
                ...
                switch (argEnumerator.Current)
                {
                    ...
                    case "--speed":
                        ...
                        break;
                    case "--brake":
                        actionsOnDevice.Add(brake);
                        break;
                    case "--coast":
                        actionsOnDevice.Add(coast);
                        break;
                    case "--current-limit":
                        actionsOnDevice.Add(laterSetCurrentLimit());
                        break;
                    case "--restore-defaults":
                    case "--restoredefaults":  // legacy name for this option from smccmd
                        actionsOnDevice.Add(restoreDefaults);
                        break;
                    case "--settings":
                    case "--set-settings":
                    case "--configure":  // legacy name for this option from smccmd
                        actionsOnDevice.Add(laterSetSettings());
                        break;
                    case "--get-settings":
                    case "--getconf":  // legacy name for this option from smccmd
                        actionsOnDevice.Add(laterGetSettings());
                        break;
                    case "--bootloader":
        ...
        private static ActionOnDevice laterGetSettings()
        {
            String filename = nextArgument();
        ...
        private static void brake(Smc device)
        {
            device.brake();
        }
        ...
        private static ActionOnDevice laterSetCurrentLimit()
        {
            UInt32 currentLimitMa = nextArgumentAsU32();
            ...
            {
                SmcSettings settings = device.getSmcSettings();
                UInt16 currentLimitPwmValue = device.findCurrentLimitPwmValue(
                    currentLimitMa,
                    settings.currentOffsetCalibration,
                    settings.currentScaleCalibration);
                device.setCurrentLimit(currentLimitPwmValue);
            };
        }
        ...
        public static UInt32 nextArgumentAsU32()
        {
            String previousArg = argEnumerator.Current;
            try
            {
                return UInt32.Parse(nextArgument());
            }
            catch (Exception exception)
            {
                throw new ArgumentException("Expected a number after \"" + previousArg + "\" between 0 and 4294967295.", exception);
            }
        }
        ...
        private static void printErrors(SmcError errors, string description)
        {
            ...
            else
            {
                ...
                if (0 != (errors & SmcError.Temperature)) Console.WriteLine("  Temperature error");
